...
func TestFirstSampleInitialization(t *testing.T) {
	...
	if info.TotalRxBytes != 2_000_000 || info.TotalTxBytes != 1_000_000 {
		t.Errorf("first sample should bootstrap totals from NIC counters: got trx=%d, ttx=%d", info.TotalRxBytes, info.TotalTxBytes)
	}
...
func TestNormalPositiveDeltas(t *testing.T) {
	...
	if info.TotalTxBytes != 150_000 || info.TotalRxBytes != 300_000 {
